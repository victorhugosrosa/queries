	...
	WHERE 1=1
		...
		AND CONVERT(DATE,DATA) between CONVERT(date,'2015-08-01') and CONVERT(date,getdate()-1)
	...
	SELECT
		...
		,(CASE
			...
			WHEN DATEPART(DW,C.DATA) = 7 THEN 'Sab'
		END) as WD_NAME
		,BI.dbo.fn_FormataVlr_Excel(SUM(C.VALOR_TOTAL)/4) AS VLR_TOTAL
		,BI.dbo.fn_FormataVlr_Excel(SUM(C.QTDE_CUPOM)/4) AS QTD_TOTAL
	FROM
		BI.DBO.BI_VENDA_CUPOM AS C
		INNER JOIN BI.dbo.BI_CAD_LOJA2 AS L
			ON C.COD_LOJA = L.COD_LOJA
	...
	WHERE 1=1
		AND C.COD_LOJA IN (29,33)
		AND CONVERT(DATE,DATA) between CONVERT(date,getdate()-35) and CONVERT(date,getdate()-8)
		AND CONVERT(DATE,DATA) <> '2015-06-01'
	GROUP BY
		L.NO_LOJA
		,ISNULL(R.TIPO_RESTAURANTE,'')
	...
	SELECT
		...
		,(CASE
			...
			WHEN DATEPART(DW,C.DATA) = 7 THEN 'Sab'
		END) as WD_NAME
		,BI.dbo.fn_FormataVlr_Excel(SUM(C.VALOR_TOTAL)/10) AS VLR_TOTAL
		,BI.dbo.fn_FormataVlr_Excel(SUM(C.QTDE_CUPOM)/10) AS QTD_TOTAL
	FROM
		BI.DBO.BI_VENDA_CUPOM AS C
		INNER JOIN BI.dbo.BI_CAD_LOJA2 AS L
			ON C.COD_LOJA = L.COD_LOJA
	...
	WHERE 1=1
		AND C.COD_LOJA IN (29,33)
		AND CONVERT(DATE,DATA) between CONVERT(date,'20150519') and CONVERT(date,'20150726')
		AND CONVERT(DATE,DATA) <> '2015-06-01'
	GROUP BY
		L.NO_LOJA
		,ISNULL(R.TIPO_RESTAURANTE,'')
