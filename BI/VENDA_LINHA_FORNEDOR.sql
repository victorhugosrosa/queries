...
from
	BI_CAD_FORNECEDOR_PRODUTO AS CFP	
	INNER JOIN BI_CAD_PRODUTO AS CP
		ON CFP.COD_PRODUTO = CP.COD_PRODUTO
	INNER JOIN BI_VENDA_PRODUTO AS VP
		ON CFP.COD_PRODUTO = VP.COD_PRODUTO
	LEFT JOIN AX2009_INTEGRACAO.dbo.TAB_CODIGO_BARRA_PRINCIPAL AS CB
		ON CFP.COD_PRODUTO = CB.COD_PRODUTO
WHERE 1=1
	AND CONVERT(DATE,VP.DATA) BETWEEN CONVERT(DATE,'2014-01-01') AND CONVERT(DATE,'2014-12-31')
	AND CFP.COD_FORNECEDOR IN (101948,102778,14648,103375,103374,14640,13828,2567,2392)
GROUP BY
	VP.COD_LOJA	
	,YEAR(VP.DATA)
	,MONTH(VP.DATA)
