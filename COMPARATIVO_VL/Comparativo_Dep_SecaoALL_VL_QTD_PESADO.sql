...
FROM
	[BI].[dbo].[BI_VENDA_GRUPO] AS VP
		...
		INNER JOIN [BI].[dbo].[BI_CAD_PRODUTO] AS CP
			ON 1 = 1
			AND VP.COD_DEPARTAMENTO = CP.COD_DEPARTAMENTO
			AND VP.COD_SECAO = CP.COD_SECAO
			AND VP.COD_GRUPO = CP.COD_GRUPO
WHERE 1 = 1
...
-- -------------------------------------------------------------------------------------------------------------------------------------------
	...
	SELECT
		...
		,SUM(QTDE_PRODUTO)/@NUM_SEM_P1 AS QTD_P2_A
	FROM
		[BI].[dbo].[BI_CAD_PRODUTO] AS CP
			INNER JOIN [BI].[dbo].[BI_VENDA_PRODUTO] AS VP
			ON CP.COD_PRODUTO = VP.COD_PRODUTO
...
-- -------------------------------------------------------------------------------------------------------------------------------------------
	...
	SELECT
		...
		,SUM(QTDE_PRODUTO)/@NUM_SEM_P1 AS QTD_P2_B
	FROM
		[BI].[dbo].[BI_CAD_PRODUTO] AS CP
			INNER JOIN [BI].[dbo].[BI_VENDA_PRODUTO] AS VP
			ON CP.COD_PRODUTO = VP.COD_PRODUTO
...
-- -------------------------------------------------------------------------------------------------------------------------------------------
	...
	SET
		...
		,MAIN.DELTA_QTD_ANO = (MAIN.QTD_P1_B / MAIN.QTD_P1_A) -1
		,MAIN.DELTA_QTD_PERIODO = (MAIN.QTD_P2_B / MAIN.QTD_P2_A) -1
	FROM
		@TAB_VENDA_SECAO AS MAIN
-- -------------------------------------------------------------------------------------------------------------------------------------------
-- SELECT
-- -------------------------------------------------------------------------------------------------------------------------------------------		
	SELECT
		...
		,BI.dbo.fn_FormataVlr_Excel(ISNULL(DELTA_QTD_ANO,0)) AS DELTA_QTD_ANO
		...
		,BI.dbo.fn_FormataVlr_Excel(ISNULL(DELTA_QTD_PERIODO,0)) AS DELTA_QTD_PERIODO
	FROM
		@TAB_VENDA_SECAO as VS
	WHERE 1 = 1
	ORDER BY
