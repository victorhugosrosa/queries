...
-- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		-- -------------------------------------------------------------------------------------			
			...
			UPDATE @TAB_VENDA_SECAO_LOJAS SET PERCENTUAL = @PERCENTUAL	WHERE COD_LOJA = @COD_LOJA AND NO_SECAO = @NO_SECAO			
	...
	SET
		CLUSTER =
		(CASE
			WHEN PERCENTUAL <= 1 AND PERCENTUAL >= 0.8 THEN 'A'
			WHEN PERCENTUAL < 0.8 AND PERCENTUAL >= 0.6 THEN 'B'
			WHEN PERCENTUAL < 0.6 AND PERCENTUAL >= 0.4 THEN 'C'
			WHEN PERCENTUAL < 0.4 THEN 'D'
		END)
